...
public class IntentUtils {
    ...
    private static final int REQUEST_CODE_ASK_PERMISSIONS_CALL_CONTACT = 1;
    ...
    public static void requestCall(Context context, String number) {
        if (!number.equals("false") && !number.equals("")) {
            ...
            if (ActivityCompat.checkSelfPermission(context, Manifest.permission.CALL_PHONE) != PackageManager.PERMISSION_GRANTED) {
                requestPermissions((Activity) context, new String[]{Manifest.permission.CALL_PHONE}, REQUEST_CODE_ASK_PERMISSIONS_CALL_CONTACT);
            } else {
                context.startActivity(intent);
            }
        }
