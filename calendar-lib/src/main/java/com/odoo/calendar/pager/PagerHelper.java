...
public class PagerHelper implements OnPageChangeListener, OnClickListener {
    ...
    private View today_day_view = null;
    ...
    private List<OdooCalendar.DateDataObject> dateDataObjects = null;
    private int range_week_of_year = 0;
    ...
    private View getView(int position, int week_of_the_year) {
        ...
        LinearLayout week_days = (LinearLayout) layout
                .findViewById(R.id.week_days);
        List<DateInfo> current_dates = mCal.getWeekDates(week_of_the_year);
        for (DateInfo date : current_dates) {
    ...
    public void onClick(View v) {
        ...
        ViewGroup currentDays = (ViewGroup) v.getParent();
        List<DateInfo> weekDates = new ArrayList<>();
        ...
        if (mOdooCalendarDateSelectListener != null) {
            dateDataObjects = mOdooCalendarDateSelectListener.weekDataInfo(
                    weekDates
            );
        }
        for (int j = 0; j < currentDays.getChildCount(); j++) {
            ...
            if (dateDataObjects != null) {
                hasData = dateDataObjects.get(j).getHasData();
            }
            if (dateInfo.isToday()) {
                if (hasData) {
                    currentDays.getChildAt(j).findViewById(R.id.day_indicator)
                            .setBackgroundResource(R.drawable.has_data_week_day);
                } else
                    currentDays.getChildAt(j).findViewById(R.id.day_indicator)
